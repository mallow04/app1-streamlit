...
with st.expander('**:violet[<아이러브 동물병원>]**'): # 서브제목
    ...
    st.write('특징) 슬개골수술 , 십자인대단열, 슬개골탈구수술, 관절고정술 등 슬개골특화 진료 병원임')
...
with st.expander('**:violet[<초콜릿>]**'): # 서브제목
    st.write('카패인과 테오브린으 함유하고 있어 강아지에게 중독을 유발할 수 있음, 소화되지 않고 신장에서 분해되어 나오기 때문에 소화기관의 문제를 일으킬 수 있음')
...
with st.expander('**:violet[<목욕하기>]**'): # 서브제목
    st.write('위생, 피부관리, 털 관리 등을 위해 필요하며 먼지와 냄새의 원인이 되는 박테리아를 제거할 수 있어 피부병, 해충을 예방하는데에 필수적임')
    st.write('https://youtu.be/zFNF-D_gEbA?si=GU31_0nx_9Adq2RN')

with st.expander('**:violet[<항문낭 짜기>]**'): # 서브제목
    st.write('항문낭 분비물이 제대로 비워지지 않을경우 통증, 염증을 유발하고 항문이 빨갛게 붓거나 지속적으로 핥는 증상을 보일 수 있음')
...
with st.expander('**:violet[<꼬리를 세우고 기지개를 필 때>]**'): # 서브제목
    st.write('놀이를 시작하자고 하는 것으로 같이 놀아달라고 하는 뜻')
